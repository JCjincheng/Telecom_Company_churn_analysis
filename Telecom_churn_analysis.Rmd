---
title: "Telecom Company Customer Churn Analysis"
  ...
  if (var_class=="factor"){
   ...
   # checkbox for percentage display
    if(input$show_percentages == TRUE){
     ...
     q<-ggplot(churn, aes(x=churn[,get(input$dist_var)])) + geom_histogram(aes(y=..density..*100),bins=10,position='dodge',color="black",fill="lightblue") +labs(title= eval(input$dist_var), x="", y="percentage(%)") 
    # checkbox for percentage display
    if(input$show_percentages == TRUE){
      q+stat_bin(aes(y=..density..*100, label=paste0(round(..density..*100,input$percentage_digits),"%")), geom="text", size=4 ,bins=10, vjust=-0.75)
    }else{
      q
    }
...
output$corrCaption <- renderText({
  paste0(input$variable1, " vs. ",input$variable2)
  })
...
output$corrPlot<-renderPlot({
  ...
  if (first_var_class=='factor' & second_var_class=='factor'){
    ...
    labs(x = eval(input$variable1),y="percentage",fill=eval(input$variable2)) +
